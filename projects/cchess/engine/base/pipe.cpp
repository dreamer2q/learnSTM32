...
extern "C" __declspec(dllexport) VOID WINAPI
    PipeOpen(PipeStruct *lppipe, LPCSTR szProcFile);
...
VOID WINAPI PipeClose(PipeStruct *lppipe) { lppipe->Close(); }
...
BOOL WINAPI PipeEof(PipeStruct *lppipe) { return lppipe->nEof != 0; }
...
void PipeStruct::Open(const char *szProcFile) {
  ...
  if (szProcFile == NULL) {
    ...
    if (CreateProcess(
            NULL, (LPSTR)szProcFile, NULL, NULL, TRUE,
            DETACHED_PROCESS | CREATE_NEW_PROCESS_GROUP | IDLE_PRIORITY_CLASS,
            NULL, NULL, &si, &pi)) {
...
void PipeStruct::ReadInput(void) {
  DWORD dwBytes;
  if (ReadFile(hInput, szBuffer + nReadEnd, LINE_INPUT_MAX_CHAR - nReadEnd,
               &dwBytes, NULL)) {
    nReadEnd += dwBytes;
    if (nBytesLeft > 0) {
      nBytesLeft -= dwBytes;
...
bool PipeStruct::CheckInput(void) {
  ...
  if (bConsole) {  // a tty, or an un-redirected handle
    ...
    return true;
  } else if (PeekNamedPipe(hInput, NULL, 0, NULL, &dwBytes,
                           NULL)) {  // a pipe without remainder data
    nBytesLeft = dwBytes;
    return nBytesLeft > 0;
...
void PipeStruct::Open(const char *szProcFile) {
  ...
  if (szProcFile == NULL) {
    ...
    pipe(nStdoutPipe);
    if (fork() == 0) {
      close(nStdinPipe[1]);   // close write
      close(nStdoutPipe[0]);  // close read

      if (dup2(nStdinPipe[0], STDIN_FILENO) < 0) {
        perror("dup stdin");
      }
      close(nStdinPipe[0]);

      if (dup2(nStdoutPipe[1], STDOUT_FILENO) < 0) {
        perror("dup stdout");
      }
      // stderr 不需要重定向
