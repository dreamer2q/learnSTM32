...
int main(void) {
  ...
  LocatePath(Search.szBookFile, "../book/BOOK.DAT");
  bPonderTime = false;
  PreGenInit();
  ...
  Search.bUseHash = Search.bUseBook = Search.bNullMove = Search.bKnowledge =
      true;
  Search.bIdle = false;
  ...
  PrintLn(
      "option idle type combo var none var small var medium var large default "
      "none");
  PrintLn(
      "option pruning type combo var none var small var medium var large "
      "default large");
  PrintLn(
      "option knowledge type combo var none var small var medium var large "
      "default large");
  PrintLn(
      "option randomness type combo var none var tiny var small var medium var "
      "large var huge default none");
        ...
        switch (UcciComm.Go) {
          ...
          case UCCI_GO_TIME_INCREMENT:
            ...
            if (UcciComm.Go == UCCI_GO_TIME_MOVESTOGO) {
              ...
              Search.nMaxTimer =
                  UcciComm.nTime * MAX(5, 11 - UcciComm.nMovesToGo) / 10;
            } else {
