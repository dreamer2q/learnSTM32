...
static bool ParsePos(UcciCommStruct &UcciComm, char *lp) {
  ...
  if (StrScanSkip(lp, " moves ")) {
    *(lp - strlen(" moves ")) = '\0';
    UcciComm.nMoveNum =
        MIN((int)(strlen(lp) + 1) / 5,
            MAX_MOVE_NUM);  // 提示："moves"后面的每个着法都是1个空格和4个字符
    for (i = 0; i < UcciComm.nMoveNum; i++) {
      dwCoordList[i] =
          *(uint32_t *)lp;  // 4个字符可转换为一个"uint32_t"，存储和处理起来方便
...
UcciCommEnum IdleLine(UcciCommStruct &UcciComm, bool bDebug) {
  ...
  if (false) {
    ...
    // 3. "position {<special_position> | fen <fen_string>} [moves
    // <move_list>]"指令
    ...
    // 7. "probe {<special_position> | fen <fen_string>} [moves
    // <move_list>]"指令
