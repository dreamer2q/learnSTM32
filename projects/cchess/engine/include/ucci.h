...
union UcciCommStruct {
   ...
   *    例如，"setoption batch
   * on"，选项类型就是"UCCI_OPTION_DEBUG"，值(Value.bCheck)就是"true"
   */
  struct {
    ...
    union {                 // 选项值
      ...
      UcciGradeEnum
          Grade;  // "combo"类型的选项"pruning"、"knowledge"和"selectivity"的值
   ...
   *    例如，position startpos moves h2e2
   * h9g8，FEN串就是"startpos"代表的FEN串，着法数(MoveNum)就是2
   */
  struct {
    const char
        *szFenStr;  // FEN串，特殊局面(如"startpos"等)也由解释器最终转换成FEN串
    int nMoveNum;  // 后续着法数
    uint32_t *
        lpdwMovesCoord;  // 后续着法，指向程序"IdleLine()"中的一个静态数组，但可以把"CoordList"本身看成数组
  };
...
UcciCommEnum IdleLine(UcciCommStruct &UcciComm,
                      bool bDebug);  // 引擎空闲时接收指令
UcciCommEnum BusyLine(
    UcciCommStruct &UcciComm,
    bool bDebug);  // 引擎思考时接收指令，只允许接收"stop"、"ponderhit"和"probe"
