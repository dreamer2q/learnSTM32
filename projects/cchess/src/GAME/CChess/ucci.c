...
struct ucci_stat u = {
    .ucciok = 0,
    .left_time = UCCI_MAXTIME,
};
int last_think_time = 0;
...
void UCCI_Parse(const char* info) {
  ...
  if (istype(type_ucciok)) {
    ...
    if (hasprefix(info, type_id_name)) {
      strcpy(u.id.name, info + sizeof type_id_name);
    } else if (hasprefix(info, type_id_version)) {
      strcpy(u.id.name, info + sizeof type_id_version);
    } else if (hasprefix(info, type_id_user)) {
      strcpy(u.id.name, info + sizeof type_id_user);
    }
    ...
    if (hasprefix(info, type_info_time)) {
      ...
      GAME_printf("cost %d", last_think_time);
    }
